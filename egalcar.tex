...
When the data to be synchronized is determined, the next step is to identify it as asset or state. This is based on an observation that different data ``behaves'' differently in MOGs and often have different requirements for synchronization. In Egal Car, information about player login and logout is identified as assets and information about a car's position and rotation is identified as state. Assets tend to be more persistent. After a player's login has been announced, this information does not need to be updated until that player logs out (or becomes disconnected due to network failure). The time interval between asset updates may be measured by seconds or even minutes. State, however, tend to be more transient. A car's position, for example, trend to change continuously. A packet containing a car's position information can be dated shortly after its generation.
